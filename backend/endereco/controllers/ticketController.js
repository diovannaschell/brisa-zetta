...
module.exports = {
  getTicketMedio: async (req, res, next) => {
    ...
    if (!moment(data, 'YYYY-MM', true).isValid()) {
      return res.status(400).send({ message: 'Invalid date format. Use YYYY-MM.' });
    }
  ...
  getClientesPorTicketMedioIntervalo: async (req, res, next) => {
    ...
    if (!data || !min || !max) {
      return res.status(400).send({ message: 'Data, min and max are required' });
    }
  ...
  getClientesComMenorTicketMedio: async (req, res, next) => {
    ...
    if (!data || !limite) {
      return res.status(400).send({ message: 'Data and limite are required' });
    }
  ...
  compareTicketMedioPeriodos: async (req, res, next) => {
    ...
    if (!data1 || !data2) {
      return res.status(400).send({ message: 'Both periods are required' });
    }
